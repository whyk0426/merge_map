...
#     min_x = min(map1.info.origin.position.x, map2.info.origin.position.x + 1)
...
#                 map2.info.origin.position.x + (map2.info.width * map2.info.resolution) + 1)
...
#             merged_x = int(np.floor((map2.info.origin.position.x + 1 + x * map2.info.resolution - min_x) / merged_map.info.resolution))
...
#             else:
#                 merged_map.data[merged_i] = (merged_map.data[merged_i] + map2.data[i]) * 0.5
#     return merged_map
...
def merge_maps(map1, map2):
    ...
    merged_map.header = map1.header
    merged_map.header.frame_id = 'map'
    min_x = min(map1.info.origin.position.x, map2.info.origin.position.x + xd)
    min_y = min(map1.info.origin.position.y, map2.info.origin.position.y + yd)
    max_x = max(map1.info.origin.position.x + (map1.info.width * map1.info.resolution),
                map2.info.origin.position.x + (map2.info.width * map2.info.resolution) + xd)    
    max_y = max(map1.info.origin.position.y + (map1.info.height * map1.info.resolution),
                map2.info.origin.position.y + (map2.info.height * map2.info.resolution)+ yd)
    merged_map.info.origin.position.x = min_x
    merged_map.info.origin.position.y = min_y
    merged_map.info.resolution = min(map1.info.resolution, map2.info.resolution)
    merged_map.info.width = int(np.ceil((max_x - min_x) / merged_map.info.resolution))
    ...
    for y in range(map2.info.height):
        for x in range(map2.info.width):
            i = x + y * map2.info.width
            merged_x = int(np.floor((map2.info.origin.position.x + xd + x * map2.info.resolution - min_x) / merged_map.info.resolution))
            merged_y = int(np.floor((map2.info.origin.position.y + yd + y * map2.info.resolution - min_y) / merged_map.info.resolution))
            merged_i = merged_x + merged_y * merged_map.info.width
            if merged_map.data[merged_i] == -1:
                merged_map.data[merged_i] = map2.data[i]
            else:
                merged_map.data[merged_i] = int((merged_map.data[merged_i] + map2.data[i]) * 0.5)
    return merged_map
